...
function Navbar() {
  ...
  return ( 
    <div className='fixed z-[999] w-full px-4 md:px-10 lg:px-20 py-4 md:py-8 flex justify-between items-center'>
      <div className='logo'>
      ...
      <div className="hidden lg:flex links gap-10">
        {["Services","Our Work","About Us","Insights","Contact"].map((item,index)=>(
          <a key={index} className={`text-lg capitalize font-light hover:text-[#CDEA68] transition-colors ${index === 4 && "ml-32"}`}>{item}</a>
        ))}
      </div>
      ...
      {/* Mobile Menu */}
      <div className={`lg:hidden fixed top-0 right-0 h-screen w-[80%] max-w-md bg-zinc-900 transform transition-transform duration-300 ease-in-out z-50 ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'} pt-20 px-6 shadow-2xl`}>
        <div className="flex flex-col gap-6">
          {["Services","Our Work","About Us","Insights","Contact"].map((item,index)=>(
            <a 
              key={index} 
              className="text-xl capitalize font-light hover:text-[#CDEA68] transition-colors"
              onClick={() => setIsMenuOpen(false)}
            >
              {item}
            </a>
